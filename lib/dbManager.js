...
DbManager.prototype.createAdapter = function createAdapter (adapterType, adapterName)
{
    ...
    if (typeof adapterType !== 'string')
        throw new $swiftErrors.TypeError('DbManager: недопустимый тип типа адаптера (ожидается: "string", принято: "' + typeof adapterType + '")');
    if (!(adapterType.toLowerCase() in adapters))
        throw new $swiftErrors.ValueError('DbManager: недопустимое значение типа адаптера (ожидается: "' + Object.keys(adapters).join('"|"') + '", принято: "' + adapterType + '")');
    //
    // создание адаптера
    //
...
DbManager.prototype.addAdapter = function addAdapter (adapter)
{
    ...
    if (!(adapter instanceof DbAdapter))
        throw new $swiftErrors.TypeError('DbManager: недопустимый тип адаптера (ожидается: "DbAdapter", принято: "' + typeof adapter + '")');

    adapterName = adapter.getName();
    adapterSetName = adapter.setName;

    if (adapterName in this._adapters)
        throw new $swiftErrors.ValueError('DbManager: адаптер с именем "' + adapterName + '" уже существует');
    //
    ...
    adapter.setName = function (name)
    {
        ...
        if (typeof name !== 'string')
            throw new $swiftErrors.TypeError('DbAdapter[' + adapterName + ']: недопустимый тип имени адаптера (ожидается: "string", принято: "' + typeof name + '")');
        if (!name.length)
            throw new $swiftErrors.ValueError('DbAdapter[' + adapterName + ']: пустое значение имени адаптера');

        if (name === adapterName) return this;

        if (name in self._adapters)
            throw new $swiftErrors.ValueError('DbAdapter[' + adapterName + ']: имя "' + name + '" уже занято другим адаптером');
        //
        // задание имени адаптеру
        //
...
DbManager.prototype.removeAdapter = function removeAdapter (adapterName, cb)
{
    ...
    {
        cb(new $swiftErrors.TypeError('DbManager: недопустимый тип имени адаптера (ожидается: "string", принято: "' + typeof adapterName+ '")'));
        return this;
    }
    if (!adapterName.length)
    {
        cb(new $swiftErrors.ValueError('DbManager: пустое значение имени адаптера'));
        return this;
    }
...
DbManager.prototype.removeAllAdapters = function removeAllAdapters (cb)
{
    ...
    (function awaiting()
    {
        immediate(function ()
        {
            ...
            if (errors.length)
            {
                cb(new $swiftErrors.MultipleError('DbManager: ошибки удаления адаптеров').setList(errors));
                return;
            }
...
DbManager.prototype.connect = function connect (cb)
{
    ...
    (function awaiting()
    {
        immediate(function ()
        {
            ...
            if (errors.length)
            {
                cb(new $swiftErrors.MultipleError('DbManager: ошибки установления соединений в адаптерах').setList(errors));
                return;
            }
...
DbManager.prototype.connectOne = function connectOne (adapterName, cb)
{
    ...
    if (!(adapterName in this._adapters))
    {
        cb(new $swiftErrors.SystemError('DbManager: адаптер с именем "' + adapterName + '" не найден'));
        return this;
    }
    //
...
DbManager.prototype.disconnect = function disconnect (cb)
{
    ...
    (function awaiting()
    {
        immediate(function ()
        {
            ...
            if (errors.length)
            {
                cb(new $swiftErrors.MultipleError('DbManager: Ошибки разрыва соединений в адаптерах').setList(errors));
                return;
            }
