...
var DbManagerError = require('./error').DbManagerError,
...
DbManager.prototype.addAdapter = function addAdapter (adapterName, adapter)
{
    if (typeof adapterName !== 'string' || !adapterName.length)
        throw new DbManagerError()
            ...
            .setCode(DbManagerError.codes.BAD_INPUT_DATA);
    ...
    this._adapters[adapterName].disconnect(function (err)
    {
        if (err)
        {
            cb(new DbManagerError()
                ...
                .setDetails(err)
                .setCode(DbManagerError.codes.REMOVE_ADAPTER_ERROR));
            return;
        }
...
DbManager.prototype.connectOne = function connectOne (adapterName, cb)
{
    ...
    this._adapters[adapterName].connect(function (err)
    {
        ...
        {
            cb(new DbManagerError()
                .setMessage('Не удалось установить соединение через адаптер "' + adapterName + '" (ответ адаптера: ' + err.message + ')')
                .setDetails(err)
                .setCode(DbManagerError.codes.CONNECT_ADAPTER_ERROR));
            return;
        }
    ...
    this._adapters[adapterName].disconnect(function (err)
    {
        if (err)
        {
            cb(new DbManagerError()
                ...
                .setDetails(err)
                .setCode(DbManagerError.codes.DISCONNECT_ADAPTER_ERROR));
            return;
        }
