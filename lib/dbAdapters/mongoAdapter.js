...
var $util = require('util'),
    ...
    MongoAdapterError = require('../errors/mongoAdapterError.js').MongoAdapterError,
...
{
    if (typeof connectionName !== 'string' || !connectionName.length) throw new MongoAdapterError()
        .setMessage('Не удалось добавить параметры соединения. Имя соединения не передано или представлено в недопустимом формате')
        .setCode(MongoAdapterError.codes.BAD_CONNECTION_NAME);
    if (connectionName in this._connectionsParams) throw new MongoAdapterError()
    ...
    if (!$swiftUtils.type.isObject(connectionParams)) throw new MongoAdapterError()
        ...
        .setCode(MongoAdapterError.codes.BAD_CONNECTION_PARAMS);
    ...
    this._connections[connectionName].close(function (err)
    {
        if (err)
        {
            cb(new MongoAdapterError()
                ...
                .setDetails(err)
                .setCode(MongoAdapterError.codes.SYSTEM_ERROR));
            return;
        }
