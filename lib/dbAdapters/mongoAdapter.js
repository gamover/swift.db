...
MongoAdapter.prototype.setConnectionsParams = function setConnectionsParams (connectionsParams)
{
    ...
    if (!$swiftUtils.type.isObject(connectionsParams))
        throw new $swiftErrors.TypeError('не удалось задать параметры соединения в адаптере "' + this._name + '". Недопустимый тип параметров соединения (ожидается: "object", принято: "' + typeof connectionsParams + '")');
    //
...
MongoAdapter.prototype.addConnectionParams = function addConnectionParams (connectionName, connectionParams)
{
    ...
    if (typeof connectionName !== 'string')
        throw new $swiftErrors.TypeError('не удалось добавить параметры соединения в адаптер "' + this._name + '". Недопустимый тип имени соединения (ожидается: "string", принято: "' + typeof connectionName + '")');
    if (!connectionName.length)
        throw new $swiftErrors.ValueError('не удалось добавить параметры соединения в адаптер "' + this._name + '". Пустое значение имени соединения');
    if (connectionName in this._connectionsParams)
        throw new $swiftErrors.ValueError('не удалось добавить параметры соединения в адаптер "' + this._name + '". Параметры соединения "' + connectionName + '" уже существуют');
    if (!$swiftUtils.type.isObject(connectionParams))
        throw new $swiftErrors.TypeError('не удалось добавить параметры соединения в адаптер "' + this._name + '". Недопустимый тип параметров соединения (ожидается: "object", принято: "' + typeof connectionParams + '")');
    //
    // добавление параметров соединения
    //
...
MongoAdapter.prototype.removeConnectionParams = function removeConnectionParams (connectionName)
{
    ...
    //
    if (typeof connectionName !== 'string')
        throw new $swiftErrors.TypeError('не удалось удалить параметры соединения из адаптера "' + this._name + '". Недопустимый тип имени соединения (ожидается: "string", принято: "' + typeof connectionName + '")');
    if (!connectionName.length)
        throw new $swiftErrors.ValueError('не удалось удалить параметры соединения из адаптера "' + this._name + '". Пустое значение имени соединения');
    //
    // удаление параметров соединения
    //
...
MongoAdapter.prototype.removeConnection = function removeConnection (connectionName, cb)
{
    ...
    if (typeof connectionName !== 'string')
    {
        cb(new $swiftErrors.TypeError('не удалось удалить соединение из адаптера "' + this._name + '". Недопустимый тип имени соединения (ожидается: "string", принято: "' + typeof connectionName+ '")'));
        return this;
    }
    if (!connectionName.length)
    {
        cb(new $swiftErrors.ValueError('не удалось удалить соединение из адаптера "' + this._name + '". Пустое значение имени соединения'));
        return this;
    }
    ...
    this._connections[connectionName].close(function (err)
    {
        if (err)
        {
            cb(new $swiftErrors.SystemError('не удалось удалить соединение "' + connectionName + '" из адаптера "' + self._name + '" (' + err.message + ')').setInfo(err));
            return;
        }
        //
        // удаление соединения
...
MongoAdapter.prototype.connectOne = function connectOne (connectionName, cb)
{
    ...
    if (typeof connectionName !== 'string')
    {
        cb(new $swiftErrors.TypeError('не удалось установить соединение в адаптере "' + this._name + '". Недопустимый тип имени сооединения (ожидается: "string", принято: "' + typeof connectionName + '")'));
        return this;
    }
    if (!connectionName.length)
    {
        cb(new $swiftErrors.ValueError('не удалось установить соединение в адаптере "' + this._name + '". Пустое значение имени сооединения'));
        return this;
    }
    if (!(connectionName in this._connectionsParams))
    {
        cb(new $swiftErrors.ValueError('не удалось установить соединение в адаптере "' + this._name + '". Соединение с именем "' + connectionName + '" не найдено'));
        return this;
    }
    //
    ...
    if (!this.isConnected(connectionName))
    {
        MongoAdapter.connect(this._connectionsParams[connectionName], function (err, connection)
        {
            if (err)
            {
                cb(new $swiftErrors.SystemError('не удалось установить соединение "' + connectionName + '" в адаптере "' + self._name + '" (' + err.message + ')').setInfo(err));
                return;
            }
...
MongoAdapter.prototype.disconnectOne = function disconnectOne (connectionName, cb)
{
    ...
    if (typeof connectionName !== 'string')
    {
        cb(new $swiftErrors.TypeError('не удалось разорвать соединение в адаптере "' + this._name + '". Недопустимый тип имени сооединения (ожидается: "string", принято: "' + typeof connectionName+ '")'));
        return this;
    }
    if (!connectionName.length)
    {
        cb(new $swiftErrors.ValueError('не удалось разорвать соединение в адаптере "' + this._name + '". Пустое значение имени сооединения'));
        return this;
    }
    if (!(connectionName in this._connections))
    {
        cb(new $swiftErrors.ValueError('не удалось разорвать соединение в адаптере "' + this._name + '". Соединение с именем "' + connectionName + '" не найдено'));
        return this;
    }
    //
    // Разрыв соединения
    ...
    if (this.isConnected(connectionName))
    {
        MongoAdapter.disconnect(this._connections[connectionName], function (err)
        {
            if (err)
            {
                cb(new $swiftErrors.SystemError('не удалось разорвать соединение "' + connectionName + '" в адаптере "' + self._name + '" (' + err.message + ')').setInfo(err));
                return;
            }
...
MongoAdapter.prototype.isConnected = function isConnected (connectionName)
{
    ...
    //
    if (typeof connectionName !== 'string')
        throw new $swiftErrors.TypeError('не удалось проверить соединение в адаптере "' + this._name + '". Недопустимый тип имени соединения (ожидается: "string", принято: "' + typeof connectionName + '")');
    if (!connectionName.length)
        throw new $swiftErrors.ValueError('не удалось проверить соединение в адаптере "' + this._name + '". Недопустимое значение имени соединения');
    //
...
MongoAdapter.connect = function connect (connectionParams, cb)
{
    try
    {
        ...
        connection.on('error', function (err) { cb(err); });
