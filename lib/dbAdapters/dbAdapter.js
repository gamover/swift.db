...
DbAdapter.prototype.setName = function setName (name)
{
    ...
    if (typeof name !== 'string')
        throw new $swiftErrors.TypeError('DbAdapter: недопустимый тип имени адаптера (ожидается: "string", принято: "' + typeof name + '")');
    if (!name.length)
        throw new $swiftErrors.ValueError('DbAdapter: пустое значение имени адаптера');
    //
    // задание имени адаптера
    //
    this._name = name;
